    ...
    def __init__(self, ch_in, ch_out, k_r , D_r: List):
        ...
        for m in range(len(D_r)):
            for l in range(len(D_r[m])):
                self.lst_block.append(nn.LeakyReLU())
                d_aux = D_r[m][l]
                assert((d_aux*(k_r-1))%2 == 0) # cond to fix the padding indipendent of kernel and dilation 
                self.lst_block.append(nn.Conv1d(ch_in, ch_out, kernel_size=k_r, dilation=D_r[m][l], padding=(d_aux*(k_r-1))//2))
...
class ResBlock(nn.Module): # no normalization in skip connection?
    ...
    def __init__(self, h_u_prev, h_u_next, k_u, k_r, D_r):
        ...
        self.mrf = MRFBlock(h_u_next, h_u_next, k_r, D_r)
    
    def forward(self, x: tensor.Torch):
        x = self.lrel(x)
        x = self.conv1(x)
        x = self.mrf(x)
        return x
...
class HIFIGAN(nn.Module):
    ...
    def __init__(self, sample_r, n_mels, h_u, k_u, k_r, D_r):
        """
        Args:
        ...
        for i in range(len(k_u)):
            self.res_block.append(ResBlock(h_u_prev, h_u_prev/2, k_u[i], k_r[i], D_r[i]))
            h_u_prev = h_u_prev/2
        self.lrel = nn.LeakyReLU() # which value init?
        self.post_conv = nn.Conv1d(h_u, n_mels, kernel_size=7, stride=1, padding=3) # pad to fix?
        self.tanh = nn.Tanh()
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = self.post_conv(x)
        x = self.tanh(x)
        x_trim = x[:, :, :self._sample_r].squeeze(1)
